...
# 
# This script reproduces the recursive out-of-sample early-warning-system
# analysis using the analysis-ready country-year panel dataset.
# 
# It reproduces:
# - the main recursive OOS performance table for the primary severe-distress
#   target (`y_true`, $(S_{it} \ge 2)$;
# - forecasting performance across alternative distress environments,
#   including early fragility $(S_{it} \ge 1)$ and purified onset distress;
# - calibration and nonlinear probability-transformation diagnostics;
# - recursive forecasting stability under bootstrap resampling;
# - Weight-of-Evidence (WoE) robustness analysis;
# - bootstrap AUC-difference tests for the main target;
# - recursive ridge-logit coefficient diagnostics;
# - correlation and variance inflation factor (VIF) diagnostics;
# - country-level distress and persistence-overlap diagnostics;
# - leave-one-country-out influence analysis;
# - robustness to the stricter distress definition $(S_{it} \ge 3)$;
# - macro-financial proxy augmentation; and
# - sensitivity of the structural model to alternative shrinkage parameters.
...
for directory in [TABLE_DIR, PRED_DIR]:
...
p_hybrid = oos_preds_t3["p_hybrid_cv"].values
...
# 
# ### TABLE 4: Recursive forecasting stability under bootstrap resampling
# 
...
mc_table_display["Mean AUC"] = (
    mc_table_display["Mean AUC"].round(3)
)

mc_table_display["SD(AUC)"] = (
    mc_table_display["SD(AUC)"].round(3)
)
...
# Save numerical version of Table 4
mc_table_display.to_csv(
    TABLE_DIR / "table_4_recursive_forecasting_stability.csv",
    index=False,
)

mc_table.to_csv(
    TABLE_DIR / "table_4_recursive_forecasting_stability_raw.csv",
    index=False,
)
print("\nTable 4: Recursive forecasting stability under bootstrap resampling")
...
table_woe_display.to_csv(TABLE_DIR / "table_5_woe_robustness_main.csv", index=False)
...
woe_coef_summary_display.to_csv(
    TABLE_DIR / "table_A2_woe_coefficient_diagnostics_display.csv",
    index=False,
)
...
print("Saved: table_A10_kappa_sensitivity.csv")
